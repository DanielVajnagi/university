...
public class MagazineCollection {
    ...
    public event MagazineListHandler MagazineAdded = default!;
    public event MagazineListHandler MagazineReplaced = default!;

    public void InvokeMagazineAddedEvent(string magazineName, int index) {
        if (CollectionName is null) return;
        var eventArgs = new MagazineListHandlerEventArgs(CollectionName, $"Magazine \"{magazineName}\" has been added to the collection", index);
        MagazineAdded.Invoke(this, eventArgs);
    }

    public void InvokeMagazineReplacedEvent(string magazineName, int index) {
        if (CollectionName is null) return;
        var eventArgs = new MagazineListHandlerEventArgs(CollectionName, $"Element has been replaced with a new \"{magazineName}\" magazine", index);
        MagazineReplaced.Invoke(this, eventArgs);
    }

    public bool Replace(int j, Magazine magazine) {
        if (j < 0 || j >= Magazines.Count) return false;

        Magazines[j] = magazine;
        InvokeMagazineReplacedEvent(Magazines[j].Name, j);
    ...
    public Magazine this[int index] {
        ...
        init {
            ...
            InvokeMagazineReplacedEvent(Magazines[index].Name, index);
        }
    }
    ...
    public void AddDefaults() {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            InvokeMagazineAddedEvent(magazine.Name, Magazines.Count - 1);
        }
        ...
        InvokeMagazineAddedEvent(forbes.Name, Magazines.Count - 1);

        var defaultMagazine = new Magazine();
        Magazines.Add(defaultMagazine);
        InvokeMagazineAddedEvent(defaultMagazine.Name, Magazines.Count - 1);
    }
